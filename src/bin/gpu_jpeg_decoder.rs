...
fn main() {
    ...
    loop {
        match file[ptr..].as_bytes() {
            [0xFF, 0xD8, ..] => {
                println!("SOI (Start Of Image [FF D8]) detected. @ {:#08X}", ptr);
                ptr += 2;
            }
            [0xFF, 0xD9, ..] => {
                println!("EOI (End Of Image [FF D8]) detected. @ {:#08X}", ptr);
                break;
            }
            [0xFF, 0xDD, later @ ..] => {
                println!("DRI (Define Restart Interval [FF DD]) detected. @ {:#08X}", ptr);
                ...
                println!("\trestart interval: {}", restart_interval.unwrap());
                ptr += 2 + segment_size as usize;
            }
            [0xFF, 0xC0, later @ ..] => {
                println!("SOF (Start Of Frame type 0(baseline) [FF C0]) detected. @ {:#08X}", ptr);
                let segment_size = u16::from_be_bytes(<[u8; 2]>::try_from(&later[..2]).unwrap());
                println!("\tsegment size: {}", segment_size);
                let sample_precision = later[2];
                println!("\tsample precision: {}", sample_precision);
                ...
                println!("\timage size: HxW: {image_height}x{image_width}");
                let number_of_components = later[7];
                println!("\tnumber of components: {}", number_of_components);
                for i in 0..number_of_components {
                    ...
                    println!("\t\tHorizontal sampling factor: {}", later[9 + i as usize * 3] >> 4);
                    println!("\t\tVertical sampling factor: {}", later[9 + i as usize * 3] & 0x0F);
                    println!("\t\tQuantization table destination selector: {}", later[10 + i as usize * 3] & 0x0F);
                }
                ptr += 2 + segment_size as usize;
            }
            [0xFF, 0xE0, later @ ..] => {
                println!("APP0 (Application type 0 segment [FF E0]) detected. @ {:#08X}", ptr);
                let segment_size = u16::from_be_bytes(<[u8; 2]>::try_from(&later[..2]).unwrap());
                println!("\tsegment size: {}", segment_size);
                assert_eq!(later[2..7], *"JFIF\0".as_bytes());
                println!("\tJFIF Ver: {:x}.{:02x}", later[7], later[8]);
                ...
                println!("\tHxW : {}x{} per unit", pixel_height, pixel_width);
                let thumb_width = later[14];
                let thumb_height = later[15];
                println!("\tthumbnail HxW : {}x{} px", thumb_height, thumb_width);
            ...
            [0xFF, 0xE1, later @ ..] => {
                ...
                println!("APP1 (Application type 1 segment [FF E1])(Exif) detected. @ {:#08X}", ptr);
                let segment_size = u16::from_be_bytes(<[u8; 2]>::try_from(&later[..2]).unwrap());
                println!("\tsegment size: {}", segment_size);
                ...
                loop {
                    ...
                    for i in 0..entry_count {
                        ...
                        println!("\t\t{:#04X}:{:?}", tag, IfdDtype::new(data_type, data_size, &val_or_offset, &later[8..]));
                    }
            ...
            [0xFF, 0xDA, later @ ..] => {
                println!("SOS (Start Of Scan segment [FF DA]) detected. @ {:#08X}", ptr);
                let segment_size = u16::from_be_bytes(<[u8; 2]>::try_from(&later[..2]).unwrap());
                println!("\tsegment size: {}", segment_size);
                let component_count = later[2];
                println!("\tnumber of components: {}", component_count);
                for i in 0..component_count {
                    ...
                    println!("\t\t DC Huffman table Number: {}", later[4 + i as usize * 2] >> 4);
                    println!("\t\t AC Huffman table Number: {}", later[4 + i as usize * 2] & 0x0F);
                }
                let start_of_spectral_selection = later[3 + component_count as usize * 2];
                println!("\tstart of spectral selection: {start_of_spectral_selection}");
                let end_of_spectral_selection = later[4 + component_count as usize * 2];
                println!("\tend of spectral selection: {end_of_spectral_selection}");
                let approx_spectral_shift = later[5 + component_count as usize * 2] >> 4;
                println!("\tapproximation spectral shift: {approx_spectral_shift}");
                let spectral_shift = later[5 + component_count as usize * 2] & 0xFF;
                println!("\tspectral shift: {spectral_shift}");
                // ptr += 2 + segment_size as usize;
            ...
            [0xFF, app_id @ 0xE0..=0xEF, later @ ..] => {
                println!("APP{0} (Application type {0} segment [FF {app_id:02X}]) detected. @ {1:#08X}", app_id - 0xE0, ptr);
                let segment_size = u16::from_be_bytes(<[u8; 2]>::try_from(&later[..2]).unwrap());
                println!("\tsegment size: {}", segment_size);
                ptr += 2 + segment_size as usize;
            }
            [0xFF, 0xDB, later @ ..] => {
                println!("DQT (Define quantization table segment [FF DB]) detected. @ {:#08X}", ptr);
                let segment_size = u16::from_be_bytes(<[u8; 2]>::try_from(&later[..2]).unwrap());
                println!("\tsegment size: {}", segment_size);
                let mut dqt_ptr = 2;
                loop {
                    let quantize_precision = later[dqt_ptr] >> 4;
                    println!("\t    quantize precision: {}", quantize_precision);
                    let quantize_table_identifier = later[dqt_ptr] & 0x0F;
                    println!("\t    quantize table id: {}", quantize_table_identifier);
                    dqt_ptr += 1;
            ...
            [0xFF, 0xC4, later @ ..] => {
                println!("DHT (Define Huffman table segment [FF C4]) detected. @ {:#08X}", ptr);
                let segment_size = u16::from_be_bytes(<[u8; 2]>::try_from(&later[..2]).unwrap());
                println!("\tsegment size: {}", segment_size);
                let mut dht_ptr = 2;
                loop {
                    ...
                    println!("\t    huffman table destination id: {}", huffman_table_id);
                    println!("\t    code counts per code length:");
                    dht_ptr += 17;
                    for (bits, code_counts) in later[dht_ptr - 16..dht_ptr].into_iter().enumerate() {
                        if *code_counts == 0 {
                            continue;
                        }
                        print!("\t\tcodes of {bits} bit(s):");
            ...
            _ => {
                println!("Unknown type segment [{:02X} {:02X}] detected. @ {:#08X}", file[ptr], file[ptr + 1], ptr);
