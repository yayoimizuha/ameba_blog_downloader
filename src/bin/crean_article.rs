...
async fn article_cleaner(a: (i64, String), b: (i64, String)) -> (String, i64) {
    ...
    (decrease_new_line.replace_all(a_text.split("\n").zip(&leave).filter_map(|(text, cond)| {
        if *cond {
            Some(text)
        } else {
            None
...
async fn main() {
    ...
    for theme in themes {
        ...
        for res in join_all(articles.into_iter().tuple_windows().map(|(a, b)| {
            ...
            let (text, id) = res.unwrap();
            sqlx::query("UPDATE blog SET article_cleaned = ? WHERE article_id = ?;")
