...
async fn inference(receiver: Receiver<(Tensor<f32>, Vec<(PathBuf, String)>)>, file_count: usize) {
    ...
    let option = SqliteConnectOptions::new().create_if_missing(true).filename(data_dir().join("face_features.sqlite"));
    let pool = SqlitePool::connect_with(option).await.unwrap();
    sqlx::query("CREATE TABLE IF NOT EXISTS features (model_hash TEXT, file_name TEXT, file_hash TEXT, feature BLOB,PRIMARY KEY (model_hash,file_name));").execute(&pool).await.unwrap();
